...
int main()
{
    ...
    if (choice == 1)
    {
        ...
        {
            if (byte1[i] > '1' || byte1[i] < '0' )
            {
                printf("There is non-valable binary\n");
                check = 0;
                break;
            }
            if (byte2[i] > '1' || byte2[i] < '0' )
            {
                printf("There is non-valable binary\n");
                check = 0;
                break;
            }
            if (byte3[i] > '1' || byte3[i] < '0' )
            {
                check = 0;
                printf("There is non-valable binary\n");
                break;
            }
            if (byte4[i] > '1' || byte4[i] < '0' )
            {
                check = 0;
                printf("There is non-valable binary\n");
                break;
        ...
        if (check != 0)
        {
            if (fstByte >= 0 && fstByte < 128)
                printf("%ld.%ld.%ld.%ld is class A\n", fstByte, scdByte, thrByte, frthByte);
            if (fstByte >= 128 && fstByte < 192)
                printf("%ld.%ld.%ld.%ld is class B\n", fstByte, scdByte, thrByte, frthByte);
            if (fstByte >= 192 && fstByte < 224)
                printf("%ld.%ld.%ld.%ld is class C\n", fstByte, scdByte, thrByte, frthByte);
            if (fstByte >= 234 && fstByte < 240)
                printf("%ld.%ld.%ld.%ld is class D\n", fstByte, scdByte, thrByte, frthByte);
            if (fstByte >= 240 && fstByte < 256)
                printf("%ld.%ld.%ld.%ld is class E\n", fstByte, scdByte, thrByte, frthByte);
        }
    ...
    if (choice == 3)
    {
        ...
        for (i = 0; i < (strlen(byte1)); i++)
        {
            ...
            if (byte2[i] >='a' && (byte2[i] <='z'))
                byte2[i] = toupper(byte2[i]);


            if (byte3[i] >='a' && (byte3[i] <='z'))
                byte3[i] = toupper(byte3[i]);


            if (byte4[i] >='a' && (byte4[i] <='z'))
